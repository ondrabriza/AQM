...
CSV_DIR = 'csv_data'        # Directory for CSV files
...
def load_configuration(target_name):
    ...
    for s in aqm_device.get('sensors', []):
        ...
        # ignore registers with address < 2 ( control and state registers)
        if reg_type == 'holding':
    ...
    # Sort sensors by register address
...
def main():
    ...
    try:
        ...
        all_sensors = holding_sensors + input_sensors # For the CSV header
    ...
    # Calculate the block range for Holding Registers
    ...
    # For 32-bit sensors like MICS_NH3_R0 (address 6), we also need to read address 7.
    # Since Thresholds end at address 10 (16-bit), we must account for the extra register.
    # This computes a safe block size for 32-bit registers at the end of the range:
    if holding_sensors:
    ...
    # Calculate the block range for Input Registers
    ...
    # Initialize CSV folder and file 
        ...
        while True:
            ...
            try:
                ...
                if not res_hold.isError() and not res_inp.isError():
                    ...
                    # Process Input registers (the rest)
                    ...
                    try:
                        ...
                        print(f"[{timestamp}] OK -> Data saved.", flush=True)
                else:
            ...
            # Write to InfluxDB if data is valid
            if data_valid:
                ...
                try:
                    ...
                    print(f"[{timestamp}] InfluxDB write error: {e}", flush=True)
